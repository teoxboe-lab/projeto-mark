...
export default function AuthPage() {
  ...
  const doLogin = async () => {
    ...
    if (error) { setErr(error.message === 'Invalid login credentials' ? 'E-mail ou senha incorretos.' : error.message); setLoading(false); return }
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg)', padding: 24 }}>
      <div style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: 12, boxShadow: 'var(--shadow-lg)', width: '100%', maxWidth: 420, padding: '36px 32px' }}>
        ...
        <p style={{ fontSize: 14, color: 'var(--text-3)', textAlign: 'center', marginBottom: 28 }}>O marketplace de contas e itens digitais do Brasil</p>
