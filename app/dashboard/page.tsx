...
export default function DashboardPage() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data: { user: u } }) => {
      ...
      if (data) { setUser(data); loadData(u.id, data.role) }
    })
  }, [])
  ...
  const totalPago = purchases.filter(p => p.amount > 0).length
    ...
    <>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '32px 24px 64px' }}>
        ...
        <div style={{ display: 'flex', borderBottom: '1px solid var(--border)', marginBottom: 24 }}>
          {[['compras', '🛒 Minhas Compras'], ['vendas', '📦 Meus Anúncios']].map(([t, l]) => (
            <button key={t} onClick={() => setTab(t as any)}
              style={{ padding: '12px 20px', fontSize: 14, fontWeight: 600, color: tab === t ? 'var(--blue)' : 'var(--text-3)', background: 'none', border: 'none', borderBottom: `2px solid ${tab === t ? 'var(--blue)' : 'transparent'}`, cursor: 'pointer', transition: 'all .15s', marginBottom: -1 }}>
              {l}
            </button>
          ))}
        </div>
